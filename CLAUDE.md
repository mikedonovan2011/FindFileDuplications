...
- `move_duplicate_file` — when `yes`, duplicates are moved to the `location_for_moved_dupes` folder for recovery instead of being left in place
...
All errors in `DuplicationRecords` (`_write_record`, `_move_record_file`, `_move_duplicate_file`, `_write_move_record`, `_calculate_hash`) raise `RuntimeError` after logging. These propagate to `main()` which catches them and exits cleanly. The same pattern applies to config loading and folder setup.
