...
If the hash hasn't been seen yet, a file is created in a "non_dupes_records" folder with:
    a) filename of <hash>.txt,
    b) the path/file of the file that was hashed as the contents of the file.

If the file with the hash already exists in the non_dupes_records folder, the file is appended
with the path/file that was hashed. It is then moved to the dupes_records folder.

If the file with the hash already exists in the dupes_records folder, the file is appended. 
