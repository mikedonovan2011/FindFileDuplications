...
class RecordRepair:
    ...
    def repair(self) -> None:
        ...
        logging.info('Checking non_dupes_records for inconsistencies')
        for record_file in self.path_for_records.non_dupes_records.glob("*.txt"):
            ...
            if line_count == 0:
                ...
                target = self.path_for_records.dupes_records / record_file.name
                logging.info(f'Repairing: {record_file.name} has {line_count} paths, moving to dupes_records')
                self._move_record_file(record_file, target)

        logging.info('Checking moved_dupes_records for inconsistencies')
        for record_file in self.path_for_records.moved_dupes_records.glob("*.txt"):
            with record_file.open("r", encoding='utf-8') as f:
                line_count = sum(1 for line in f if line.strip())

            if line_count == 0:
                logging.info(f'Deleting empty moved_dupes_records record: {record_file.name}')
                self._delete_record(record_file)
