...
def setup_env(tmp_path, **config_overrides):
    """Create config, folders, and return (configs, folders) for testing."""
...
def test_first_occurrence_not_moved(tmp_path):
    configs, folders = setup_env(tmp_path)
...
def test_dupes_folder_empty_in_move_mode(tmp_path):
    ...
    assert list(folders.record_folder_paths.dupes_records.glob("*.txt")) == []
...
def test_move_disabled_does_not_move(tmp_path):
    configs, folders = setup_env(tmp_path, move='no')
    scan_dir = tmp_path / 'scan'
    file1 = create_test_file(scan_dir / 'a', 'photo.jpg')
    file2 = create_test_file(scan_dir / 'b', 'photo.jpg')
    ...
    assert list(folders.record_folder_paths.moved_dupes_records.glob("*.txt")) == []
    # moved_dupes_files should be empty
    assert list(folders.moved_dupes_files_path.rglob("*")) == []
